...
namespace MaterialUI
{
    public class SelectionBoxConfig : MonoBehaviour
    {
        ...
        [Header("List item options")] public bool rippleEnabled = true;
        ...
        [Range(0, 3)] public int expandedListShadowLevel = 2;

        [Space(12f)] public string[] listItems;

        [Header("List options")] public bool autoMaxItemHeight;
        ...
        [Space(12f)] public GameObject listLayer;
        ...
        private void Update()
        {
            ...
            if (state == 1)
            {
                ...
                {
                    // Fade text out
                    var tempColor = selectedText.color;
                    ...
                    // Make list expand
                    var tempVec2 = thisRect.sizeDelta;
            ...
            else if (state == 2)
            {
                if (animDeltaTime <= animationDuration)
                {
                    ...
                    // AnimColor list
                    thisImage.color =
                        Anim.Quint.In(currentColor, contractedListColor, animDeltaTime, animationDuration);

                    // Fade text out
                    listCanvasGroup.alpha =
                        Anim.Quint.Out(listCanvasAlpha, 0f, animDeltaTime, animationDuration * 0.6f);
        ...
        public void Setup()
        {
            ...
            for (var i = 0; i < listItems.Length; i++)
            {
                ...
                listItem.transform.localPosition = new Vector3(listItem.transform.localPosition.x,
                    listItem.transform.localPosition.y, 0f);
                listItem.transform.localRotation = new Quaternion(0f, 0f, 0f, 0f);
                listItem.GetComponentInChildren<Text>().text = listItems[i];

                var tempConfig = listItem.GetComponent<SelectionListItemConfig>();
                tempConfig.listId = i;

                var tempRippleConfig = tempConfig.GetComponent<RippleConfig>();
        ...
        public void ExpandList()
        {
            ...
            {
                var tempFloat = Screen.height / 100f * percentageOfScreenHeight / 36f;
