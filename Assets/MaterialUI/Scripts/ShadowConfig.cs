...
{
    public class ShadowConfig : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler,
        IPointerExitHandler
    {
        ...
        [Range(0, 3)] public int shadowNormalSize = 1;

        [Range(0, 3)] public int shadowActiveSize = 2;
        ...
        public void SetShadows(int shadowOn)
        {
            if (isEnabled)
            {
                foreach (var shadow in shadows) shadow.SetShadow(false);

                if (shadowOn - 1 >= 0) shadows[shadowOn - 1].SetShadow(true);
            }
        }
    }
}