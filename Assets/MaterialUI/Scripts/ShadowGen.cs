...
namespace MaterialUI
{
    ...
    public class ShadowGen : MonoBehaviour
    {
        ...
        [Range(0, 5)] public int blurRange = 3;
        ...
        [HideInInspector] public Sprite sourceSprite;

        [HideInInspector] public Texture2D sourceTex;

        [HideInInspector] public Texture2D destTex;

        [HideInInspector] public Sprite destSprite;

        [HideInInspector] public Image destImage;

        //	Unwise to change this
        private readonly float darkenShadow = 0.97f;
        ...
        private readonly int imagePadding = 128;
        ...
        private void Start()
        {
            ...
            if (destSprite) destImage.sprite = destSprite;
        }
        ...
        public void GenerateShadowFromImage()
        {
            ...
            if (sourceImage)
            {
                ...
                {
                    textureFileName = string.Format(@"Assets/MaterialUI/GeneratedShadows/{0}.png", Guid.NewGuid());
        ...
        private void Setup()
        {
            ...
            var widthWithPadding = sourceTex.width + imagePadding * 2;
            var heightWithPadding = sourceTex.height + imagePadding * 2;
            ...
            while (i < blurIterations)
            {
                while (xCounter < destTex.width)
                {
                    while (yCounter < destTex.height)
                    {
                        if (blurRange == 1)
                        {
                            pixCol.a = destTex.GetPixel(xCounter, yCounter - 1).a / 4 +
                                       destTex.GetPixel(xCounter, yCounter).a / 2 +
                                       destTex.GetPixel(xCounter, yCounter + 1).a / 4;
                            pixCol.r = 0;
                            pixCol.g = 0;
                            pixCol.b = 0;
                        ...
                        {
                            pixCol = destTex.GetPixel(xCounter, yCounter - 2) +
                                     destTex.GetPixel(xCounter, yCounter - 1) + destTex.GetPixel(xCounter, yCounter) +
                                     destTex.GetPixel(xCounter, yCounter + 1) +
                                     destTex.GetPixel(xCounter, yCounter + 2);
                            destTex.SetPixel(xCounter, yCounter, pixCol / (darkenShadow * 5));
                        }
                        else if (blurRange == 3)
                        {
                            pixCol = destTex.GetPixel(xCounter, yCounter - 3) +
                                     destTex.GetPixel(xCounter, yCounter - 2) +
                                     destTex.GetPixel(xCounter, yCounter - 1) + destTex.GetPixel(xCounter, yCounter) +
                                     destTex.GetPixel(xCounter, yCounter + 1) +
                                     destTex.GetPixel(xCounter, yCounter + 2) +
                                     destTex.GetPixel(xCounter, yCounter + 3);
                            destTex.SetPixel(xCounter, yCounter, pixCol / (darkenShadow * 7));
                        }
                        else if (blurRange == 4)
                        {
                            pixCol = destTex.GetPixel(xCounter, yCounter - 4) +
                                     destTex.GetPixel(xCounter, yCounter - 3) +
                                     destTex.GetPixel(xCounter, yCounter - 2) +
                                     destTex.GetPixel(xCounter, yCounter - 1) + destTex.GetPixel(xCounter, yCounter) +
                                     destTex.GetPixel(xCounter, yCounter + 1) +
                                     destTex.GetPixel(xCounter, yCounter + 2) +
                                     destTex.GetPixel(xCounter, yCounter + 3) +
                                     destTex.GetPixel(xCounter, yCounter + 4);
                            destTex.SetPixel(xCounter, yCounter, pixCol / (darkenShadow * 9));
                        }
                        else if (blurRange == 5)
                        {
                            pixCol = destTex.GetPixel(xCounter, yCounter - 5) +
                                     destTex.GetPixel(xCounter, yCounter - 4) +
                                     destTex.GetPixel(xCounter, yCounter - 3) +
                                     destTex.GetPixel(xCounter, yCounter - 2) +
                                     destTex.GetPixel(xCounter, yCounter - 1) + destTex.GetPixel(xCounter, yCounter) +
                                     ...
                                     destTex.GetPixel(xCounter, yCounter + 3) +
                                     destTex.GetPixel(xCounter, yCounter + 4) +
                                     destTex.GetPixel(xCounter, yCounter + 5);
                ...
                while (xCounter < destTex.width)
                {
                    while (yCounter < destTex.height)
                    {
                        if (blurRange == 1)
                        {
                            pixCol.a = destTex.GetPixel(xCounter - 1, yCounter).a / 4 +
                                       destTex.GetPixel(xCounter, yCounter).a / 2 +
                                       destTex.GetPixel(xCounter + 1, yCounter).a / 4;
                            pixCol.r = 0;
                            pixCol.g = 0;
                            pixCol.b = 0;
                            destTex.SetPixel(xCounter, yCounter, pixCol / darkenShadow);
                        ...
                        {
                            pixCol = destTex.GetPixel(xCounter - 2, yCounter) +
                                     destTex.GetPixel(xCounter - 1, yCounter) + destTex.GetPixel(xCounter, yCounter) +
                                     destTex.GetPixel(xCounter + 1, yCounter) +
                                     destTex.GetPixel(xCounter + 2, yCounter);
                            destTex.SetPixel(xCounter, yCounter, pixCol / (darkenShadow * 5));
                        }
                        else if (blurRange == 3)
                        {
                            pixCol = destTex.GetPixel(xCounter - 3, yCounter) +
                                     destTex.GetPixel(xCounter - 2, yCounter) +
                                     destTex.GetPixel(xCounter - 1, yCounter) + destTex.GetPixel(xCounter, yCounter) +
                                     destTex.GetPixel(xCounter + 1, yCounter) +
                                     destTex.GetPixel(xCounter + 2, yCounter) +
                                     destTex.GetPixel(xCounter + 3, yCounter);
                            destTex.SetPixel(xCounter, yCounter, pixCol / (darkenShadow * 7));
                        }
                        else if (blurRange == 4)
                        {
                            pixCol = destTex.GetPixel(xCounter - 4, yCounter) +
                                     destTex.GetPixel(xCounter - 3, yCounter) +
                                     destTex.GetPixel(xCounter - 2, yCounter) +
                                     destTex.GetPixel(xCounter - 1, yCounter) + destTex.GetPixel(xCounter, yCounter) +
                                     destTex.GetPixel(xCounter + 1, yCounter) +
                                     destTex.GetPixel(xCounter + 2, yCounter) +
                                     destTex.GetPixel(xCounter + 3, yCounter) +
                                     destTex.GetPixel(xCounter + 4, yCounter);
                            destTex.SetPixel(xCounter, yCounter, pixCol / (darkenShadow * 9));
                        }
                        else if (blurRange == 5)
                        {
                            pixCol = destTex.GetPixel(xCounter - 5, yCounter) +
                                     destTex.GetPixel(xCounter - 4, yCounter) +
                                     destTex.GetPixel(xCounter - 3, yCounter) +
                                     destTex.GetPixel(xCounter - 2, yCounter) +
                                     destTex.GetPixel(xCounter - 1, yCounter) + destTex.GetPixel(xCounter, yCounter) +
                                     ...
                                     destTex.GetPixel(xCounter + 3, yCounter) +
                                     destTex.GetPixel(xCounter + 4, yCounter) +
                                     destTex.GetPixel(xCounter + 5, yCounter);
                            destTex.SetPixel(xCounter, yCounter, pixCol / (darkenShadow * 11));
                        }
        ...
        private void ApplyChanges()
        {
            ...
            MaterialGlobals.shadowSpriteBorder = new Vector4(sourceSprite.border.w + imagePadding,
                sourceSprite.border.x + imagePadding, sourceSprite.border.y + imagePadding,
                sourceSprite.border.z + imagePadding);
            ...
            destImage.rectTransform.sizeDelta = new Vector2(sourceImage.rectTransform.sizeDelta.x + imagePadding * 2,
                sourceImage.rectTransform.sizeDelta.y + imagePadding * 2);
            ...
            var tempRect = destImage.rectTransform.position + shadowRelativePosition;
            destImage.rectTransform.position = tempRect;

            //		Resizes the shadow Image
            var tempVec2 = destImage.rectTransform.sizeDelta + shadowRelativeSize;
            destImage.rectTransform.sizeDelta = tempVec2;

            //		Makes the shadow Image the desired transparency
            var tempColor = destImage.color;
