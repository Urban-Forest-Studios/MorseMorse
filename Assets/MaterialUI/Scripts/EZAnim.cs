...
namespace MaterialUI
{
    ...
    {
        [HideInInspector] public List<EZStruct> theStructs;
        ...
        private void Start()
        {
            ...
            for (var i = 0; i < theStructs.Count; i++)
            {
                ...
                if (tempStruct.valueType == ValType.Property)
                    tempStruct.realProperty = tempStruct.targetGameObject.GetComponent(tempStruct.targetComponent)
                        .GetType().GetProperty(tempStruct.targetVariable);


                if (tempStruct.methodOnEnd)
                {
                    tempStruct.methodRealComponent =
                        tempStruct.methodTargetGameObject.GetComponent(tempStruct.methodTargetComponent);
                    tempStruct.methodRealMethod = tempStruct.methodTargetGameObject
                        .GetComponent(tempStruct.methodTargetComponent).GetType()
                        .GetMethod(tempStruct.methodTargetMethod);
                }
            ...
            for (var i = 0; i < theStructs.Count; i++)
                if (activeList[i])
                {
                    ...
                    if (tempStruct.animDeltaTime < tempStruct.animationDuration)
                    {
                        ...
                        {
                            tempFloat = UpdateAnimation(tempStruct.initialFloat, tempStruct.targetFloat,
                                tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);
                        ...
                        {
                            tempFloat = UpdateAnimation(tempStruct.initialInt, tempStruct.targetInt,
                                tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.valueType == ValType.Field)
                                tempStruct.realField.SetValue(tempStruct.RealComponent, Mathf.RoundToInt(tempFloat));
                            else if (tempStruct.valueType == ValType.Property)
                                tempStruct.realProperty.SetValue(tempStruct.RealComponent, Mathf.RoundToInt(tempFloat),
                                    null);
                        }
                        else if (tempStruct.variableType == "UnityEngine.Vector2")
                        {
                            ...
                            if (tempStruct.modifyParameter1)
                                tempVector2.x = UpdateAnimation(tempStruct.initialVector2.x, tempStruct.targetVector2.x,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter2)
                                tempVector2.y = UpdateAnimation(tempStruct.initialVector2.y, tempStruct.targetVector2.y,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);
                        ...
                        else if (tempStruct.variableType == "UnityEngine.Vector3")
                        {
                            ...
                            if (tempStruct.modifyParameter1)
                                tempVector3.x = UpdateAnimation(tempStruct.initialVector3.x, tempStruct.targetVector3.x,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter2)
                                tempVector3.y = UpdateAnimation(tempStruct.initialVector3.y, tempStruct.targetVector3.y,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter3)
                                tempVector3.z = UpdateAnimation(tempStruct.initialVector3.z, tempStruct.targetVector3.z,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);
                        ...
                        else if (tempStruct.variableType == "UnityEngine.Rect")
                        {
                            ...
                            if (tempStruct.modifyParameter1)
                                tempRect.x = UpdateAnimation(tempStruct.initialRect.x, tempStruct.targetRect.x,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter2)
                                tempRect.y = UpdateAnimation(tempStruct.initialRect.y, tempStruct.targetRect.y,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter3)
                                tempRect.width = UpdateAnimation(tempStruct.initialRect.width,
                                    tempStruct.targetRect.width, tempStruct.animDeltaTime, tempStruct.animationDuration,
                                    tempStruct.animationType);

                            if (tempStruct.modifyParameter4)
                                tempRect.height = UpdateAnimation(tempStruct.initialRect.height,
                                    tempStruct.targetRect.height, tempStruct.animDeltaTime,
                                    tempStruct.animationDuration, tempStruct.animationType);
                        ...
                        else if (tempStruct.variableType == "UnityEngine.Color")
                        {
                            ...
                            if (tempStruct.modifyParameter1)
                                tempColor.r = UpdateAnimation(tempStruct.initialColor.r, tempStruct.targetColor.r,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter2)
                                tempColor.g = UpdateAnimation(tempStruct.initialColor.g, tempStruct.targetColor.g,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter3)
                                tempColor.b = UpdateAnimation(tempStruct.initialColor.b, tempStruct.targetColor.b,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter4)
                                tempColor.a = UpdateAnimation(tempStruct.initialColor.a, tempStruct.targetColor.a,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);
                        ...
                        else if (tempStruct.variableType == "UnityEngine.Material")
                        {
                            ...
                            if (tempStruct.modifyParameter1)
                                tempColor.r = UpdateAnimation(tempStruct.initialColor.r, tempStruct.initialColor.r,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter2)
                                tempColor.g = UpdateAnimation(tempStruct.initialColor.g, tempStruct.initialColor.g,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter3)
                                tempColor.b = UpdateAnimation(tempStruct.initialColor.b, tempStruct.initialColor.b,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);

                            if (tempStruct.modifyParameter4)
                                tempColor.a = UpdateAnimation(tempStruct.initialColor.a, tempStruct.initialColor.a,
                                    tempStruct.animDeltaTime, tempStruct.animationDuration, tempStruct.animationType);
        ...
        private void AnimStruct(int i)
        {
            ...
            else if (tempStruct.variableType == "UnityEngine.Vector2")
            {
                if (tempStruct.valueType == ValType.Field)
                    ...
                    tempStruct.initialVector2 =
                        (Vector2)tempStruct.realProperty.GetValue(tempStruct.RealComponent, null);
            }
            else if (tempStruct.variableType == "UnityEngine.Vector3")
            {
                if (tempStruct.valueType == ValType.Field)
                    ...
                    tempStruct.initialVector3 =
                        (Vector3)tempStruct.realProperty.GetValue(tempStruct.RealComponent, null);
            }
            else if (tempStruct.variableType == "UnityEngine.Rect")
            {
                if (tempStruct.valueType == ValType.Field)
            ...
            else if (tempStruct.variableType == "UnityEngine.Material")
            {
                if (tempStruct.valueType == ValType.Field)
                    ...
                    tempStruct.initialMaterial =
                        (Material)tempStruct.realProperty.GetValue(tempStruct.RealComponent, null);
            }
        ...
        {
            for (var i = 0; i < theStructs.Count; i++) AnimStruct(i);
        }
        ...
        private IEnumerator delayedCall()
        {
            ...
            object[] parametersArray = { oldStruct.methodParam };

            oldStruct.methodRealMethod.Invoke(oldStruct.methodRealComponent, parametersArray);
        }

        private float UpdateAnimation(float initialValue, float targetValue, float time, float duration,
