...
namespace MaterialUI
{
    public class ScreenConfig : MonoBehaviour
    {
        ...
        [HideInInspector] [SerializeField] public TransitionType transitionInType;

        [HideInInspector] [SerializeField] public TransitionType transitionOutType;

        [HideInInspector] public SlideDirection slideIn = SlideDirection.Bottom;

        [HideInInspector] public float slideInPercent = 25f;

        [HideInInspector] public bool scaleIn = true;

        [HideInInspector] public float scaleInStartValue = 0.75f;

        [HideInInspector] public bool fadeIn = true;

        [HideInInspector] public float fadeInStartValue;

        [HideInInspector] public SlideDirection slideOut = SlideDirection.Bottom;

        [HideInInspector] public float slideOutPercent = 25f;

        [HideInInspector] public bool scaleOut = true;

        [HideInInspector] public float scaleOutEndValue = 0.75f;

        [HideInInspector] public bool fadeOut = true;

        [HideInInspector] public float fadeOutEndValue;
        ...
        [HideInInspector] private float animDeltaTime;

        [HideInInspector] private float animStartTime;

        private ScreenConfig hideScreen;

        [HideInInspector] private float rippleSize;

        [HideInInspector] private Vector2 screenDimensions;

        [HideInInspector] private Vector2 screenSpacePosition;

        [HideInInspector] private Vector2 slideInDirectionPosition;

        [HideInInspector] private Vector2 slideOutDirectionPosition;

        [HideInInspector] private int state;

        [HideInInspector] private Vector2 tempVector2;

        [HideInInspector] private Vector3 tempVector3;

        [HideInInspector] private CanvasGroup theCanvasGroup;

        [HideInInspector] private RectTransform theRectTransform;
                ...
                if (animDeltaTime <= animationDuration)
                {
                    ...
                    else
                    {
                        if (slideIn != SlideDirection.None)
                        {
                            ...
                            tempVector2.x = Anim.Quint.Out(slideInDirectionPosition.x, 0f, animDeltaTime,
                                animationDuration);
                            tempVector2.y = Anim.Quint.Out(slideInDirectionPosition.y, 0f, animDeltaTime,
                                animationDuration);
                        ...
                        if (fadeIn)
                            theCanvasGroup.alpha =
                                Anim.Quint.Out(fadeInStartValue, 1f, animDeltaTime, animationDuration);
                    }
                }
                else
                {
                    ...
                    {
                        if (slideIn != SlideDirection.None) theRectTransform.anchoredPosition = new Vector2(0f, 0f);

                        if (scaleIn) theRectTransform.localScale = new Vector3(1f, 1f, 1f);

                        if (fadeIn) theCanvasGroup.alpha = 1f;
                    }
            ...
            else if (state == 2)
            {
                if (animDeltaTime <= animationDuration)
                {
                    ...
                    else
                    {
                        ...
                        {
                            tempVector2 = theRectTransform.anchoredPosition;
                            tempVector2.x = Anim.Quint.Out(0f, slideOutDirectionPosition.x, animDeltaTime,
                                animationDuration);
                            tempVector2.y = Anim.Quint.Out(0f, slideOutDirectionPosition.y, animDeltaTime,
                                animationDuration);
                            theRectTransform.anchoredPosition = tempVector2;
                        }
                        ...
                        if (fadeOut)
                            theCanvasGroup.alpha =
                                Anim.Quint.Out(1f, fadeOutEndValue, animDeltaTime, animationDuration);
                    }
                }
                else
                {
